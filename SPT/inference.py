...
from modules.utils import set_random_seed, noise_sample, recover_data, split_num_cat_target
...
def main():
    ...
    artifact = wandb.use_artifact(
        f"{project}/{model_name}_{config['ver']}:v0",
        type='model')
    ...
    output_path = f"./syn_data/synthetic_prior_{model_name}_{config['seed']}.csv"
    ...
    #%%
    syn_df.to_csv(f"./syn_data/{model_name}_{config['seed']}_{config['latent_noise_variance']}")
