...
class Tokenizer(nn.Module):
    def __init__(self, d_numerical, categories, d_token, bias):
        ...
        if categories is None:
            ...
            # print("category_offsets:",category_offsets)
            self.category_embeddings = nn.Embedding(sum(categories), d_token)
            # print("sum(categories):",sum(categories))
            nn_init.kaiming_uniform_(self.category_embeddings.weight, a=math.sqrt(5))
            print("From Tokenizer in AE, when 'self.categories' is not None")
...
class AE(nn.Module):
    def __init__(self, d_numerical, categories, num_layers, hid_dim, n_head = 1, factor = 32, bias = True):
...
class Encoder_model(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head, factor, bias = True):
        ...
        self.AE_Encoder = Transformer(num_layers, d_token, n_head, d_token, factor)

    def load_weights(self, Pretrained_AE):
        self.Tokenizer.load_state_dict(Pretrained_AE.AE.Tokenizer.state_dict())
        self.AE_Encoder.load_state_dict(Pretrained_AE.AE.encoder.state_dict())
...
class Decoder_model(nn.Module):
    def __init__(self, num_layers, d_numerical, categories, d_token, n_head, factor, bias = True):
        super().__init__()
        self.AE_Decoder = Transformer(num_layers, d_token, n_head, d_token, factor)
        self.Detokenizer = Reconstructor(d_numerical, categories, d_token)
        
    def load_weights(self, Pretrained_AE):
        self.AE_Decoder.load_state_dict(Pretrained_AE.AE.decoder.state_dict())
