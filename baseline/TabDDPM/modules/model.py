...
class GaussianMultinomialDiffusion(nn.Module):
    def __init__(
        ...
        if config["multinomial_loss_type"] == 'vb_all':
        ...
        self.register_buffer('Lt_history', torch.zeros(config["num_timesteps"]))
        self.register_buffer('Lt_count', torch.zeros(config["num_timesteps"]))
